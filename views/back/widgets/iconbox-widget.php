...
class VeuseIconboxWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
	 		'veuse_infobox_widget', // Base ID
			__('Icon-box (Veuse)','veuse-uikit'), // Name
			array( 'description' => __( 'Add a styled box with a FontAwesome icon', 'veuse-uikit' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $title ) )
			...
			echo do_shortcode('[veuse_iconbox title="'.$title.'" text="'.$text.'" href="'.$button_link .'" buttontext="'. $button_text .' " style="'.$style.'" bordercolor="'.$bordercolor.'"  background="'.$backgroundcolor.'" color="'.$textcolor.'" icon="'.$icon.'"]');
	...
	public function form( $instance ) {
		...
		if ( isset( $instance[ 'text' ] ) ) $text = $instance[ 'text' ];	else $text = __( 'Enter text here', 'text_domain' );		
		...
		<textarea class="widefat" rows="5" name="<?php echo $this->get_field_name( 'text' ); ?>"><?php echo esc_attr($text);?></textarea></p>
		...
		<p>
			...
			<select name="<?php echo $this->get_field_name('style');?>">
		  		...
		  		<option value="blue" <?php selected( $style, 'blue' , true); ?>><?php _e('Blue','veuse-pagelist');?></option>
		  		<option value="lightblue" <?php selected( $style, 'lightblue' , true); ?>><?php _e('Light blue','veuse-pagelist');?></option>
...
add_action('widgets_init',create_function('','return register_widget("VeuseIconboxWidget");'));
